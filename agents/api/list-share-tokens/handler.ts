...
/**
 * GET /projects/{id}/shares — list non-revoked share tokens for a project.
 *
 * Response 200: { tokens: ShareToken[] }
...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    return {
      statusCode: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ tokens: queryResult.Items ?? [] }),
    };
  } catch (err) {
    console.error(
      JSON.stringify({
        message: "Failed to list share tokens",
        projectId,
