...
1. **Categories are CLOSED.** Each slot's \`category\` MUST be exactly one of these 12 canonical values — no synonyms, no plural forms, no novel categories:
   ...
   \`hero | testimonial | footer | cta | faq | contact | navigation | stats | motion | carousel | layout/grid | layout/split\`

2. **Page reads top-to-bottom.** Order the slots in the order they will appear on the rendered page. A typical order is:
   \`navigation\` → \`hero\` → body sections (\`stats\`, \`layout/grid\`, \`layout/split\`, \`testimonial\`, \`carousel\`, \`motion\`, \`faq\`) → \`cta\` → \`footer\`.
...
6. **Adapt the count and mix to the brief.** Service businesses lean toward FAQ + CTA; portfolios lean toward carousel/grid; stores lean toward grid + stats; about/institutional pages lean toward layout/split + testimonial.
...
[
  {
    "category": "<one of: hero | testimonial | footer | cta | faq | contact | navigation | stats | motion | carousel | layout/grid | layout/split>",
...
export function buildSkeletonUserPrompt(
  ...
  const pageTypeSection = sellerHint
    ? [
        "## Page Type Hint",
        "",
        `The seller has indicated this should be a **${sellerHint}** page. Adapt the skeleton mix accordingly (e.g. \`store\` favors grid + stats; \`portfolio\` favors carousel/grid; \`services\` favors layout/split + faq + cta; \`about\` favors layout/split + testimonial; \`landing\` favors hero + cta-driven flow).`,
      ].join("\n")
    : "";
